...
//namespace Func_forex_Ravi_Oanda_Api.Algorithms
//{
...
//                    if (orderPlaced)
//                        log.LogInformation($"{DateTime.Now}: {instrument_name}: Trade Placed at Bid({latestCandle.Bid.O}):Close{latestCandle.Mid.C}:Ask{latestCandle.Ask.O}, Units {orderRequest.order.units}");
//                    else
//                        log.LogInformation($"{DateTime.Now}: {instrument_name}: Trade Place Failed at Bid({latestCandle.Bid.O}):Close{latestCandle.Mid.C}:Ask{latestCandle.Ask.O}, Units {orderRequest.order.units}");

//                    return await oandaApi.GetAccount();
//                }
//            }
//            log.LogInformation($"{DateTime.Now}: {instrument_name}: Trade Skipped, EMA5/20/50 Previous:{ema_5_previous}/{ema_20_previous}/{ema_50_previous} Current: {ema_5_current}/{ema_20_current}/{ema_50_current}");
//            return accountDetail;
//        }
//    }
//}